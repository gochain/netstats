...
History.prototype.add = function (block, id, trusted, addingHistory) {
	...
	if (!_.isUndefined(block) && !_.isUndefined(block.number) && !_.isUndefined(block.uncles) && !_.isUndefined(block.transactions) && !_.isUndefined(block.difficulty) && block.number > 0) {
		...
		if (historyBlock) {
			...
			if (this._items.length === 0 || (this._items.length === MAX_HISTORY && block.number > this.worstBlockNumber()) || (this._items.length < MAX_HISTORY && block.number > this.bestBlockNumber() && addingHistory) || (this._items.length < MAX_HISTORY && block.number < this.worstBlockNumber() && addingHistory) ) {
		...
		var chartHistory = _(this._items)
			...
			.map(function (item) {
				return {
					...
					transactionrate: (item.block.transactions.length > 0 && item.block.time > 0) ? (item.block.transactions.length / (item.block.time / 1000)).toFixed(0) : 0,
