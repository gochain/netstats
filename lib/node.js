...
Node.prototype.getInfo = function(callback)
{
	return {
		...
		stats: {
			...
			queued: this.stats.queued, //TODO follow pending around and add queued everyqhere?
		},
...
Node.prototype.setStats = function(stats, history, callback)
{
	if( !_.isUndefined(stats) )
	{
		...
		this.setQueued( _.result(stats, 'queued', this.stats.queued), function (err, stats) {} );
...
Node.prototype.setQueued = function(stats, callback)
{
    if( !_.isUndefined(stats) && !_.isUndefined(stats.queued))
    {
        if(!_.isEqual(stats.queued, this.stats.queued))
        {
